...
module.exports = {
			...
			subCommand
				...
				.addBooleanOption((option) =>
					option
						...
						.setDescriptionLocalizations({
							'sv-SE':
								'Växlar om det här meddelandet ska vara kortvarigt eller inte',
						}),
				),
		)
		.addSubcommand((subCommand) =>
	 ...
	 */
	async execute(_BOT, _DATABASE, Interaction) {
		...
		if (COMMAND === 'attachment') {
			...
			await PGP('decrypt', { passphrase: PASS, file: FILE }).then(
				async (data) => {
					let mdata = data
					if (mdata) {
						...
						if (mdata.replace(' ', '').length <= 0)
							return await Interaction.editReply(
								'There is no data associated with this file.',
							)

						return await Interaction.editReply({ content: mdata })
					}
				},
				async (error) => {
					// biome-ignore lint:
			...
			return Interaction.reply({
				content: 'Please specify either the modal option or submit a file.',
				ephemeral: true,
			})
		}
	},
