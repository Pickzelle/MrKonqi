	...
	async execute(_BOT, _DATABASE, Interaction) {
		...
		if (maxMessagesToDelete > 300)
			return await Interaction.reply({
				content:
					'Sorry, but I will not handle deletion requests with an amount over `300`.',
				ephemeral: true,
			})
		...
		while (messagesDeleted < maxMessagesToDelete) {
			...
			if (targetUser)
				messagesToDelete = fetchedMessages.filter(
					(msg) => msg.author.id === targetUser.user.id,
				)
			else if (webhook)
				messagesToDelete = fetchedMessages.filter(
					(msg) => msg.author.id === webhook,
				)
			...
			try {
				const deletedMessages = await Interaction.channel.bulkDelete(
					messagesToDelete.first(maxMessagesToDelete),
				)
