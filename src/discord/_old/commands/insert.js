...
module.exports = {
	...
	async execute(BOT, DATABASE, Interaction) {
		...
		if (channel.parent.id !== SUGGESTIONS_CHANNEL_ID)
			return await Interaction.reply({
				content: `This command can only be executed for <#${SUGGESTIONS_CHANNEL_ID}> channels.`,
				ephemeral: true,
			})
		...
		insertEntry(BOT, DATABASE, 'supportThreads', channel, status).then(
			async () => {
				await Interaction.editReply({
					content: `The database entry ${channel.id} has been inserted with the status ${status}.`,
				})
			},
			console.error,
		)
