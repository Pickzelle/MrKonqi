...
const { insertEntry, deleteEntry, updateEntry } = require(
...
module.exports = {
	 ...
	 */
	async execute(Bot, Database, _config, oldThread, newThread) {
				...
				Database.get(
					...
					(err, row) => {
						if (err) {
							...
							updateEntry(Bot, Database, 'suggestions', newThread).catch(
								(err) => {
									// biome-ignore lint:
									console.log(err)
								},
							...
							insertEntry(Bot, Database, 'suggestions', newThread).catch(
								(err) => {
									// biome-ignore lint:
									console.log(err)
								},
