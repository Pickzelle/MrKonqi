		...
		if (trimmedLine.startsWith('%') && trimmedLine.endsWith('%')) {
			...
			if (currentKey === 'NAME') {
				...
				const isExcludedLine =
					trimmedLine.includes('.zst') || trimmedLine.includes('000755')
				...
				if (!isExcludedLine) {
					...
					if (
						...
						packages[currentPkgName][currentKey] =
							packages[currentPkgName][currentKey] || []
						packages[currentPkgName][currentKey].push(
	...
	try {
		...
		await Promise.all(
			gzipFiles.map(async (gzipFile) => {
				try {
					const fileContent = await extractGzip(
						PATH.join(OS.tmpdir(), 'MrKonqi', gzipFile),
					)
					const packages = parsePackages(fileContent)

					const jsonFilename = gzipFile.replace('.db', '.json')
					await writeFile(
						PATH.join(dbDir, jsonFilename),
