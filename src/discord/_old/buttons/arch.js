...
const ARCH = new AttachmentBuilder(
...
module.exports = {
	...
	async execute(_BOT, Interaction) {
		...
		const packagee = Interaction.message.embeds[0].data.description
		...
		const { lastFetchTimestamp } = JSON.parse(
			readFileSync(PATH.join(__dirname, '../stored', 'timestamp.json')),
		)

		// Read the JSON file for the specific repository.
		const file = readFileSync(
			PATH.join(__dirname, `../stored/${repo}.json`),
			'utf-8',
		)
		const parsed = JSON.parse(file)
		const packageNames = extractPackageName(packagee)
		...
		if (packageNames[0] in parsed) {
			...
			if (packageData.ARCH)
				description = `${description}**Architecture:** ${packageData.ARCH}\n`
			if (repo)
				description = `${description}**Repository:** ${repo.charAt(0).toUpperCase() + repo.slice(1)}\n`
			if (packageData.DESC)
				description = `${description}**Description:** ${packageData.DESC}\n`
			if (packageData.URL)
				description = `${description}**Upstream URL:** ${packageData.URL}\n`
			if (packageData.LICENSE)
				description = `${description}**License:** ${packageData.LICENSE}\n`

			// Use template literals for providing arrays in the description.
			if (packageData.PROVIDES)
				description += `**Provides:** ${packageData.PROVIDES.map((item) => `\`${item}\``).join(' ')}\n`
			if (packageData.REPLACES)
				description += `**Replaces:** ${packageData.REPLACES.map((item) => `\`${item}\``).join(' ')}\n`
			if (packageData.CONFLICTS)
				description += `**Conflicts:** ${packageData.CONFLICTS.map((item) => `\`${item}\``).join(' ')}\n`

			if (packageData.CSIZE)
				description = `${description}**Package Size:** ${formatSize(packageData.CSIZE)}\n`
			if (packageData.ISIZE)
				description = `${description}**Installed Size:** ${formatSize(packageData.ISIZE)}\n`
			...
			if (packageData.BUILDDATE)
				description = `${description}**Build Date:** <t:${packageData.BUILDDATE}:R>\n\n`

			// Check if dependencies should be shown.
			const showDepends = extractNonBreakingSpace(packagee)

			if (showDepends) {
				if (packageData.DEPENDS) {
					const dependsPackages = `${packageData.DEPENDS}`
						.split(',')
						.map((packageName) => `\n${packageName}`)
						.join(' ')
					FIELDS.push({
				...
				if (packageData.OPTDEPENDS) {
					const optDependsPackages = `${packageData.OPTDEPENDS}`
						.split(',')
						.map((packageName) => `\n${packageName}`)
						.join(' ')
					FIELDS.push({
				...
				if (packageData.MAKEDEPENDS) {
					const makeDependsPackages = `${packageData.MAKEDEPENDS}`
						.split(',')
						.map((packageName) => `\n${packageName}`)
						.join(' ')
					FIELDS.push({
			...
			await Interaction.update({
				...
				components: [],
				files: [],
				content: 'The package is not in the selected repository any longer.',
				ephemeral: true,
			})
		}
	},
