 ...
 */
const updateMsg = async function(BOT, DATABASE, thread) {
	...
	DATABASE.serialize(() => {
		DATABASE.get('SELECT * FROM suggestions WHERE id = ?', [thread.id], (err) => {
			if (err) {
		...
		database.each(
			...
			() => {
				...
				try {
					...
					featureTracker.send({ embeds: [EMBED], files: [LOGO] }).catch(err => console.log(err));
				}
			},
		);
	}
