...
const updateMsg = require(PATH.join(__dirname, '..', 'modules', 'updateFeatureRequest.js'));
...
function updateEntry(BOT, DATABASE, TABLE, channel) {
	...
	return new Promise((resolve, reject) => {
		...
		if (channel.parent.id !== PARENTID) return reject(red('[Database] - Parent is incorrect'));

		DATABASE.run(`UPDATE ${TABLE} SET status = ? WHERE id = ?`, ['wait', channel.id], (updateErr) => {
			if (updateErr) {
...
function insertEntry(BOT, DATABASE, TABLE, channel, status) {
	...
	return new Promise((resolve, reject) => {
		...
		DATABASE.run(`CREATE TABLE IF NOT EXISTS ${TABLE} (id TEXT PRIMARY KEY, status TEXT)`);
		DATABASE.run(
...
function deleteEntry(BOT, DATABASE, table, channel) {
	...
	return new Promise((resolve, reject) => {
		...
		DATABASE.serialize(() => {
			DATABASE.run(`CREATE TABLE IF NOT EXISTS ${table} (id TEXT PRIMARY KEY, status TEXT)`);
			DATABASE.get(`SELECT * FROM ${table} WHERE id = ?`, [channel.id], (err, row) => {

				if (err) {
					console.log(yellowBright(`[Database] - ${channel.id} not found in database.`));
				}
				else if (row) {
					DATABASE.run(`DELETE FROM ${table} WHERE id = ?`, [channel.id], (deleteErr) => {
						if (deleteErr) {
					...
					reject(yellowBright('[Database] - The post was archived and has since been deleted.'));
				}
				else {
